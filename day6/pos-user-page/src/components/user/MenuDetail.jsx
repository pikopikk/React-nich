...
export default function MenuDetail() {
  ...
  if (!menu) {
    return <h1>Menu tidak ditemukan</h1>;
  }
      ...
      <div className="flex flex-col items-start">
        ...
        <div className="flex flex-col px-5 py-4 items-start w-full gap-1 bg-gradient-to-l from-[#F1F0E4] to-white">
          <div className="flex flex-row items-center gap-2 text-base text-stone-900">
            ...
            {tersedia ? (
              ...
              <RiCloseCircleFill className="w-5 h-5 text-error" />
            )}
          </div>

          <JaminanMenu />

          <h4 className="text-base text-stone-900">Kategori Menu:</h4>
          <div className="flex flex-row gap-2">
            ...
            {isBest && (
              <div className="badge badge-outline badge-primary text-[#747934]">
                Best
              </div>
            )}
            {isSignature && (
              <div className="badge badge-outline badge-secondary text-[#c99b5f]">
                Signature
              </div>
            )}
          </div>
        ...
        <div className="bg-[#F1F0E4] text-stone-900 py-4 px-5">
          <div className="bg-white rounded-2xl py-4 px-5 shadow-md">
            <h4 className="text-base font-semibold">Deskripsi Menu:</h4>
...
function MenuTidakTersedia(){
  return(
    <span className="bg-[#3E3F29] text-white absolute left-0 bottom-0 text-base py-1 px-2.5 rounded-tr-2xl">
      Tidak Tersedia
    </span>
  )
}
...
function MenuDetailButton({ menu }) {
  const { addToCart, cart } = useCart(); // ambil fungsi dari context
  const navigate = useNavigate();
  ...
  const handleOrderNow = () => {
    // kalau item belum ada di cart -> tambahkan
    const exists = cart.find((i) => i.id === menu.id);
    if (!exists) {
      addToCart(menu);
    }
    navigate("/Keranjang");
  ...
  return (
    <div className="flex flex-row h-20 items-center justify-between gap-0 fixed bottom-0 left-0 w-full p-5 bg-stone-900 border-t border-base-300">
      ...
      >
        ...
        <p className="text-white">Cara Pesan</p>
      </Link>
      ...
      <button
        ...
        className="flex justify-center items-center w-[30%] btn-press"
      >
      ...
      <button
        ...
        disabled={!menu.tersedia}
        className="flex flex-col items-center justify-center w-[35%] gap-1 text-white btn-press"
      >
    ...
    <>
      ...
      <div className="modal" role="dialog">
        <div className="modal-box bg-white shadow-md text-sm text-stone-900">
          <h3 className="text-base font-bold">Jaminan Menu Kami</h3>
